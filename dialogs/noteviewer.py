...
class NoteViewer(DialogTemplate):
    def __init__(self, main_window, notes_folder_path):
        ...
        self.load_notes()
        self.pack_frames({self.locationComboBox:[1,0.1,0,0],
    ...
    def load_notes(self, event=None):
        ...
        self.notes = oslistdir(ospathjoin(self.notesFolderPath, self.locationComboBox.get()))
        if self.notes == []:
            return
        for index,noteFile in enumerate(self.notes):
            ...
            with open(ospathjoin(self.notesFolderPath, self.locationComboBox.get(), noteFile), "r") as file:
