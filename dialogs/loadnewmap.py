...
class LoadNewMapDialog(DialogTemplate):
    ...
    def __init__(self, main_window, data_directory):
        ...
        self.bottomRightCoordLabel = Label(self.labelFrame, text="Bottom Right Coordinate:")
        #Add items to grid
    ...
    def on_okay(self):
        ...
        if not self.bottomRightCoordEntry.state() ==():
            return
        tileServers = {"map":"https://mt0.google.com/vt/lyrs=m&hl=en&x={x}&y={y}&z={z}&s=Ga", "satellite":"https://mt0.google.com/vt/lyrs=s&hl=en&x={x}&y={y}&z={z}&s=Ga"}
        ...
        self.progressLabel = Label(self.top, text="Downloading maps...", anchor="center")
        self.progressbar = Progressbar(self.top,variable=self.progressPercent, mode="determinate", maximum=100)
        ...
        for server in tileServers:
            ...
            self.loaders.append(OfflineLoader(path=database_path, tile_server=tileServers[server]))
            self.loaders[-1].save_offline_tiles((topLeftLat+0.01, topLeftLong-0.01), (bottomRightLat-0.01, bottomRightLong+0.01), 6, 19)
        self.percent = 0
        self.top.after(100, self.update_progress_bar)
        
        with open(ospathjoin(self.dataDirectory, name,"db","data.csv"), "w", newline="") as csvfile:
