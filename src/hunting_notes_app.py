...
class HuntingNotesApp:
    ...
    def __init__(self, main_window, logger, weather_fields, species_classes):
        ...
        self.dataDirectory = utils.resource_path(
            "Property Data", file_name=__file__
        )  # for app making, comment line below

        # Class Data
        self.markers = []
        self.currentMarker = None
        self.huntDate = None
        self.huntStartTime = None
        self.huntLength = None
        ...
        if self.settings.get("Theme") is not None:
            Style().theme_use(self.settings["Theme"])
        else:  # If no style is listed in the settings, default to darkly instead
            self.logger.warning('Theme not selected, defaulting to "darkly"')
            Style().theme_use("darkly")

        # Select the property to be used
        if self.settings.get("Last Map") is not None:
            ...
            if not ospathexists(
                self.databaseFolder
            ):  # Check that the database actually exists or we need to change it
                self.change_property()
            else:
                self.notesPath = ospathjoin(self.dataDirectory, self.database, "notes")
                self.imagesPath = ospathjoin(
                    self.dataDirectory, self.database, "pictures"
                )
                self.create_map()
        else:  # If the "Last Map" setting doesn't exist, select a new property
            self.change_property()

        # Get the units the system should be in
        if self.settings.get("Imperial") is not None:
            if self.settings.get("Imperial"):
                self.units = "Imperial"
            else:
                self.units = "Metric"
        else:
            self.units = "Imperial"
            self.settings.update({"Imperial": True})
            
        #Get which species we were hunting last time
        if self.settings.get("Species") is None:
            self.desiredSpecies = "Deer"
            self.settings.update({"Species":"Deer"})
        else:
            self.desiredSpecies = self.settings["Species"]
            

        # Options menu
        settingsIconPath = ospathjoin(
            utils.resource_path("", __file__), "Resources", "settings_icon.png"
        )
        self.optionBar = OptionBar(
            self.root,
            side="right",
            frame_place_properties={"relheight": 0.15, "relwidth": 0.2},
            expand_button_properties={"image": settingsIconPath, "zoom": 0.03},
            expand_button_place_properties={
                "relx": 0.02,
                "rely": 0.02,
                "relwidth": 0.05,
                "relheight": 0.05,
        ...
        self.optionBar.add_combobox(
            "Theme",
            Style().theme_names(),
            self.settings["Theme"],
            self.check_theme,
            self.themeVariable,
            place_properties={"relheight": 0.3},
        )
        self.desiredSpeciesVariable = StringVar(value=self.settings["Species"])
        self.optionBar.add_combobox(
            "Hunt Species",
            [key for key in self.speciesClasses.keys() if not "human" in key.lower()],
            self.settings["Species"],
            self.change_hunt_species,
            self.desiredSpeciesVariable,
            place_properties={"relheight": 0.3},
        )
        self.satelliteVar = BooleanVar(value=self.settings["Satellite"])
        self.optionBar.add_checkbutton(
            "Satellite",
            self.change_map_type,
            properties={
                "style": "Roundtoggle.Toolbutton",
                "variable": self.satelliteVar,
            },
            place_properties={"relx": 0.4},
        )
        self.unitVar = BooleanVar(value=self.settings["Imperial"])
        self.optionBar.add_checkbutton(
            "Imperial Units",
            self.change_units,
            properties={
        ...
        # Hunt
        self.sidebar.add_menu_tab("Hunt", tab_place_properties={"relheight": 0.2})
        ...
        self.sidebar.add_entry(
            "Add Marker", "New Marker Coordinates:", "Markers", self.add_marker
        )
        # Notes menus
        ...
        self.sidebar.add_menu_button(
            "Delete Abandoned Notes", self.delete_abandoned_notes, "Notes"
        )
        # Change property
        self.sidebar.add_menu_tab("Property", tab_place_properties={"relheight": 0.1})
        self.sidebar.add_menu_button(
            "Change Property", self.change_property, "Property"
        )
        # Retrain
        self.sidebar.add_menu_tab(
            "Train Models", tab_place_properties={"relheight": 0.2}
        )
        self.sidebar.add_menu_button(
            "Train Animal Detector", self.train_animal_detector, "Train Models"
        )
        self.sidebar.add_menu_button(
            "Train Animal Finder", self.train_animal_finder, "Train Models"
        )
        # Extras
        self.sidebar.add_menu_tab("Extras", tab_place_properties={"relheight": 0.1})
        self.sidebar.add_menu_button(
            "Image Annotator", self.start_image_annotator, "Extras"
        )
        # Help
        self.sidebar.add_menu_tab("Help", tab_place_properties={"relheight": 0.1})
        self.sidebar.add_menu_button("Tutorial", self.open_help, "Help")
        self.sidebar.add_menu_button("Documentation", self.open_documentation, "Help")

        self.root.bind("<Button-1>", self.left_click_event)
    ...
    def create_map(self):
        ...
        if self.settings["Satellite"]:
            self.map_widget = TkinterMapView(
                ...
                width=1000,
                height=700,
                database_path=ospathjoin(self.databaseFolder, "satellite.db"),
            ...
            self.map_widget = TkinterMapView(
                ...
                width=1000,
                height=700,
                database_path=ospathjoin(self.databaseFolder, "map.db"),
                use_database_only=True,
                max_zoom=19,
            )
            self.map_widget.set_tile_server(
        ...
        self.map_widget.add_right_click_menu_command(
            label="Add Marker", command=self.add_marker, pass_coords=False
        )
        self.map_widget.add_right_click_menu_command(
            label="Delete Marker", command=self.delete_marker, pass_coords=False
        )
    ...
    def setup_map(self):
        ...
        """
        with open(
            ospathjoin(self.dataDirectory, self.databaseFolder, "data.csv")
        ) as csvfile:
            ...
            self.homePosition = (
                float(row[0]),
                float(row[1]),
            )  # Keep the home position since we need this for the weather data
            row = reader.__next__()
            self.boundingBox = (
            ...
            for i in range(0, len(row), 2):
                self.propertyLineCorners.append(
                    tuple([float(row[i]), float(row[i + 1])])
                )
        self.map_widget.set_position(*self.homePosition)
        self.map_widget.set_zoom(
            10
        )  # This is a hack to get the images to work properly and not get image tearing
        self.map_widget.fit_bounding_box(*self.boundingBox)
        if not self.settings["Satellite"]:
            self.propertyBorder = self.map_widget.set_polygon(
                self.propertyLineCorners, name="PropertyBorder", fill_color=None
            )
    ...
    def load_markers(self):
        ...
        try:
            with open(
                ospathjoin(self.dataDirectory, self.databaseFolder, "markers.csv")
            ) as csvfile:
                reader = csvreader(csvfile)
                for row in reader:
                    if not row == []:
            ...
            with open(
                ospathjoin(self.dataDirectory, self.databaseFolder, "markers.csv"), "w"
            ) as csvfile:
                pass

    def left_click_event(self, event):
        """
        Called when the user left clicks the mouse (Only used to deselect a marker really since selecting one is done through the creation of the marker in the markers module)

        :param event: The event that happened to call this function
        :type event: tkinter.event
        """
        # We only need this to run if we are trying to select or deselect markers, any menus or options shouldn't affect marker highlighting
        if not str(type(event.widget)) == "<class 'tkinter.Canvas'>":
            return
    ...
    def add_marker(self, event=None):
        ...
        if event is not None:
            ...
            self.valCoordToolTip = ToolTip(
                event.widget, "Format: latitude, longitude (eg. -42.5464, -83.5734)"
            )
        ...
        newMarker = Marker(
            self.map_widget,
            markerDialog.result["lat"],
            markerDialog.result["long"],
            markerDialog.result["name"],
            markerDialog.result["markerType"],
            self.dataDirectory,
            self.database,
        )
        self.markers.append(newMarker)
        ...
        if not ospathexists(
            ospathjoin(
                self.dataDirectory, self.database, "notes", markerDialog.result["name"]
            )
        ):
            osmkdir(
                str(
        ...
        with open(
            ...
            writer.writerow(
                [
                    str(markerDialog.result["lat"]),
                    str(markerDialog.result["long"]),
                    markerDialog.result["name"],
                    markerDialog.result["markerType"],
                ]
            )  # Add the marker to the data file
    ...
    def delete_marker(self):
        ...
        markers = []
        if (
            self.currentMarker is None
        ):  # If there is no current marker selected, do nothing
            return
        ...
        if not ospathexists(
            ospathjoin(self.dataDirectory, self.database, "pictures", "abandoned")
        ):
            osmkdir(
                ospathjoin(self.dataDirectory, self.database, "pictures", "abandoned")
            )
        if not ospathexists(
            ospathjoin(self.dataDirectory, self.database, "notes", "abandoned")
        ):
            osmkdir(ospathjoin(self.dataDirectory, self.database, "notes", "abandoned"))
        # Check for pictures folder and remove it
        if ospathexists(
            ospathjoin(self.dataDirectory, self.database, "pictures", markerName)
        ):
            if (
                not len(
                    oslistdir(
                        ospathjoin(
                            self.dataDirectory, self.database, "pictures", markerName
                        )
                    )
                )
                == 0
                ...
                for file in oslistdir(
                    ...
                    shutilmove(
                        ...
                        ospathjoin(
                            self.dataDirectory, self.database + "_pictures", "abandoned"
                        ),
                    )
            osrmdir(
                ospathjoin(self.dataDirectory, self.database, "pictures", markerName)
        ...
        if ospathexists(
            ospathjoin(self.dataDirectory, self.database, "notes", markerName)
        ):
            if (
                not len(
                    oslistdir(
                        ospathjoin(
                            self.dataDirectory, self.database, "notes", markerName
                        )
                    )
                )
                == 0
                ...
                for file in oslistdir(
                    ...
                    shutilmove(
                        ospathjoin(
                            self.dataDirectory, self.database, "notes", markerName, file
                        ),
                        ospathjoin(
                            self.dataDirectory, self.database + "_notes", "abandoned"
                        ),
                    )
            osrmdir(ospathjoin(self.dataDirectory, self.database, "notes", markerName))
        # Read current markers from file
        with open(
            ospathjoin(self.dataDirectory, self.databaseFolder, "markers.csv"), "r"
        ) as csvfile:
            reader = csvreader(csvfile)
            for row in reader:
                if not row == []:
                    markers.append(row)
            markers.pop(
                self.markers.index(self.currentMarker)
        ...
        self.markers.pop(
            self.markers.index(self.currentMarker)
        )  # Remove the currently selected marker from the list
    ...
    def add_images(self):
        ...
        if self.currentMarker is None:
            messagebox.showerror(
                "No Marker Selected",
                "Please select a marker before trying to add images",
            )
            return
        if not self.currentMarker.type == "Camera":
            ...
            return
        AddMarkerImagesDialog(self.root, self.currentMarker)
    ...
    def go_hunt(self):
        ...
        self.infoBox = InfoBox(
            self.root, "Loading Model Data", "Please wait while model library loads"
        )
        from animal_regression.animal_finder import (
            AnimalFinder,
        )  # Dynamically import the library, used to get faster startup time until you try to go hunting

        self.infoBox.close_info_box()

        self.huntDialog = HuntDialog(
            self.root,
            self.huntDate,
            self.huntStartTime,
            self.huntLength,
        ...
        self.huntDate = self.huntDialog.result["Date"]
        self.huntStartTime = self.huntDialog.result["Start Time"]
        self.huntLength = self.huntDialog.result["Time Length"]

        self.infoBox = InfoBox(
            self.root, "Loading Model Data", "Please wait while model data loads"
        )
        self.finder = AnimalFinder(
            self.root,
            self.dataDirectory,
            self.database,
            self.weatherFields,
        ...
        if self.finder.isLoading:
            self.finderLoaderTimer = utils.RepeatTimer(
                1, self.check_finder_loaded, ["Repeating"]
            )
    ...
    def predict_and_process(self):
        ...
        self.infoBox = InfoBox(
            self.root,
            "Predicting",
            "Please wait while model predicts the best time and location",
        )
        predictions = self.finder.predict(
            self.huntDialog.result["Date"],
            self.huntDialog.result["Time Length"],
            self.timeInterval,
        )
        ...
        for marker in self.markers:
            if marker.type == "Stand" or marker.type == "Point of Interest":
                distance = haversine.haversine(
                    best_camera_coords, (marker.lat, marker.long), unit="mi"
                )
    ...
    def train_animal_detector(self):
        ...
        """
        self.detectorInfoBox = InfoBox(
            self.root,
            "Training Animal Detector",
            "Please wait while detector data loads",
        )
    ...
    def train_animal_finder(self):
        ...
        """
        self.finderInfoBox = InfoBox(
            self.root, "Training Animal Finder", "Please wait while finder data loads"
        )
        from animal_regression.animal_finder import (
            AnimalFinder,
        )  # Dynamically import the library, used to get faster startup time until you try to go hunting
    ...
    def open_help(self):
        """
        Opens up the help documentation for the app
        """
        webbrowser.open_new_tab(
            "file://"
            + ospathjoin(
                utils.resource_path("", __file__), "Documentation", "tutorial.html"
            )
        )

    def open_documentation(self):
        """
        Opens the code documentation
        """
        webbrowser.open_new_tab(
            "file://"
            + ospathjoin(
                utils.resource_path("", __file__), "Documentation", "index.html"
            )
        )
    ...
    def change_units(self):
        """
        Changes the units between imperial and metric
        """
        self.settings["Imperial"] = self.unitVar.get()
        if self.settings["Imperial"]:
            self.units = "Imperial"
        else:
            self.units = "Metric"
        self.save_settings("Imperial", self.unitVar.get())
        
    def change_hunt_species(self):
        """
        Changes which species will be used when hunting
        """
        self.desiredSpecies = self.desiredSpeciesVariable.get()
    ...
    def save_settings(self, setting_to_change=None, value=None):
        ...
        """
        filePath = ospathjoin(
            utils.resource_path("", file_name=__file__), "settings.txt"
        )
