...
class Sidebar:
    ...
    def __init__(
        ...
        min_rel_detect,
        max_rel_display,
        min_rel_display: float = 0,
        side: str = "left",
        default_tab_height: float = 0.1,
        animate: bool = False,
    ):
        self.root = root
        ...
        self.expandedFrame = [
            None,
            None,
        ]  # Holds the current frame to be filled and the last one that needs to be removed
        ...
        rootNode = SidebarWidget(
            "Sidebar",
            Frame,
            self.root,
            widget_place_properties={
                "relwidth": self.currentSize,
                "relheight": 1.0,
                "relx": self.sidebarXStart,
                "anchor": self.anchor,
                "rely": 0,
            },
        )
    ...
    def check_in_sidebar(self, event):
        ...
        if not self.sidebarExpanded:  # If the sidebar is collapsed
            if (
                abs(
                    (event.x_root - self.root.winfo_x()) / self.root.winfo_width()
                    - self.sidebarXStart
                )
                < self.minDetect
            ):  # Check if it is inside in the x direction
                self.expand()
        else:
            if (
                abs(
    ...
    def expand(self):
        ...
        if not self.isAnimated:
            ...
            if self.currentSize < self.sidebarMaxWidth:
                self.currentSize = self.currentSize + self.changeIncrement * (
                    self.sidebarMaxWidth - self.sidebarMinWidth
                )
    ...
    def label_clicked(self, event):
        ...
        for frame in self.widgetTree.children("Sidebar"):
            if frame.widget == event.widget:
                if (
                    frame.tag.replace("Tab", "WidgetFrame") == self.expandedFrame[1]
                ):  # Check if we already clicked on the already expanded tab
    ...
    def add_widget(
        ...
        :type type:
        """
        newNode = SidebarWidget(
            name,
            type,
            self.widgetTree[menu_name],
            properties.copy(),
            place_properties.copy(),
        )
    ...
    def add_menu_tab(
        self,
        name: str,
        tab_expandable: bool = True,
        tab_properties: dict = {},
        tab_place_properties: dict = {},
    ):
        """
        ...
        self.add_widget(
            name + "WidgetFrame", Frame, place_properties=tabPlaceProperties
        )
    ...
    def add_menu_button(
        ...
        buttonProperties.update(
            {"command": button_command, "text": name, "style": "TButton"}
        )
    ...
    def add_combobox(
        ...
        self.add_widget(
            ...
            Combobox,
            tabname + "WidgetFrame",
            properties={"values": combo_values, "textvariable": combo_variable},
            place_properties={
                "relwidth": 0.5,
                "relheight": 1.0,
                "relx": 0.5,
    ...
    def add_entry(
        ...
        self.add_widget(
            ...
            Label,
            tabname + "WidgetFrame",
            properties={"text": label_text, "anchor": "center"},
        )
        entryProperties = properties.copy()
        placeProperties = place_properties.copy()
        self.add_widget(
            name, Entry, tabname + "WidgetFrame", entryProperties, placeProperties
        )
        self.widgetTree[name].widget.bind(
            "<Button-1>", lambda event: self.switch_entry_binds(event, command_func)
        )
        self.place_children(tabname + "WidgetFrame")
    ...
    def place_children(self, tab_tag):
        ...
        for child in tabChildren:
            if numChildrenRelHeight > 0:
                widgetRelHeight = (1 - childRelTotal) / (
                    len(tabChildren) - numChildrenRelHeight
                )
            else:
