...
class AnimalDetectorTrainingDialog(DialogTemplate):
    ...
    def __init__(self, root_window):
        ...
        self.epochLabel = Label(
            self.widgetFrame, text="Training Epochs", anchor="center"
        )
        ...
        if torchcudaavailable():
            ...
            self.deviceComboBoxValues = {
                torchcudagetdevicename(item): item
                for item in range(0, torchcudadevicecount())
            }
        ...
        self.epochBox.place(
            relwidth=1.0, relheight=relativeHeights, rely=relativeHeights
        )
        self.batchLabel.place(
            relwidth=1.0, relheight=relativeHeights, rely=2 * relativeHeights
        )
        self.batchBox.place(
            relwidth=1.0, relheight=relativeHeights, rely=3 * relativeHeights
        )
        if relativeHeights == 1 / 6:
            self.deviceLabel.place(
                relwidth=1.0, relheight=relativeHeights, rely=4 * relativeHeights
            )
            self.deviceComboBox.place(
                relwidth=1.0, relheight=relativeHeights, rely=5 * relativeHeights
            )

        self.pack_frames(
            {self.widgetFrame: [1.0, 0.8, 0, 0], self.buttonFrame: [1.0, 0.2, 0, 0.8]}
        )
        self.show()
    ...
    def on_okay(self):
        ...
        if not self.batchBox.state() == ():
            return
        self.result = {
            "Batch Size": int(self.batchBox.get()),
        ...
        if self.device == "cpu":
            ...
            self.result.update(
                {"Device": self.deviceComboBoxValues[self.deviceComboBox.get()]}
            )
        self.close_dialog()
