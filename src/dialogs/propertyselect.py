...
class PropertySelectDialog(DialogTemplate):
    ...
    def __init__(self, main_window, data_directory):
        ...
        # Widgets
        self.loadNewMapButton = Button(
            self.top, text="Load new map", command=self.load_new_map
        )
        ...
        self.pack_frames(
            {
                self.widgetFrame: [1, 1 - 2 * self.relativeButtonHeight, 0, 0],
                self.buttonFrame: [
                    1,
                    self.relativeButtonHeight,
                    0,
                    1 - 2 * self.relativeButtonHeight,
                ],
                self.loadNewMapButton: [
                    1,
                    self.relativeButtonHeight,
                    0,
                    1 - self.relativeButtonHeight,
                ],
            }
        )
    ...
    def load_map_options(self):
        ...
        if files == []:  # If there is no data, then force a map loading session
            messagebox.showwarning(
                "No Map Data",
                "There are no maps currently downloaded, please download one now",
            )
            self.load_new_map()
        files = [
            f for f in files if not f.endswith(".csv")
        ]  # Remove any files that have csv at end
        files = [
            f.replace("_", " ") for f in files
