...
class AddMarkerDialog(DialogTemplate):
    ...
    def __init__(self, main_window, coords=None):
        ...
        if coords is None:
            ...
            self.latLongLabel.place(
                relwidth=1.0, relheight=entryRelHeight, rely=entryRelHeight
            )
            self.typeLabel.place(
                relwidth=1.0, relheight=comboboxRelHeight, rely=2 * entryRelHeight
            )
        else:
        ...
        if coords is None:
            ...
            self.latLongEntry.place(
                relwidth=1.0, relheight=entryRelHeight, rely=entryRelHeight
            )
            self.typeEntry.place(
                relwidth=1.0, relheight=comboboxRelHeight, rely=2 * entryRelHeight
            )
        else:
        ...
        if coords is None:
            ...
            self.pack_frames(
                {self.widgetFrame: [1, 0.6, 0, 0], self.buttonFrame: [1, 0.4, 0, 0.6]}
            )
        # Tooltip
        self.helpToolTip = ToolTip(
            self.latLongEntry, "Format: latitude, longitude (eg. -42.5464, -83.5734)"
        )
        self.show()
    ...
    def on_okay(self):
        ...
        long = float(
            coord[1].strip()
        )  # Replace any spaces with nothing before trying to convert it
