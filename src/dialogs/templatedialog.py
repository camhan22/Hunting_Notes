...
class DialogTemplate:
    ...
    def __init__(
        self, main_window, is_modal: bool = True, no_okay_cancel: bool = False
    ):
        ...
        if not no_okay_cancel:
            self.okButton = Button(
                self.buttonFrame,
                text="Ok",
                command=self.on_okay,
                style="okcancel.TButton",
            )
    ...
    def pack_frames(self, widget_list: dict = {}):
        ...
        for item in widget_list:
            item.place(
                relwidth=widget_list[item][0],
                relheight=widget_list[item][1],
                relx=widget_list[item][2],
                rely=widget_list[item][3],
            )
    ...
    def show(self, no_wait=False):
        ...
        self.position = (
            "+"
            + str(
                int(self.mainWindow.winfo_screenwidth() / 2)
                - int(self.top.winfo_width() / 2)
            )
            + "+"
            + str(
                int(self.mainWindow.winfo_screenheight() / 2)
                - int(self.top.winfo_height() / 2)
            )
        )
