...
class HuntingNotesApp():
    ...
    :raises Keyrror: Happens if the settings.txt file doesn't have Last Map or Theme in it
    If the Theme is missing, it defaults to darkly
    If the Last Map is missing, it will bring up the property select dialog
    """
    ...
    :raises FileNotFoundError: If this happens, an empty markers.csv file is created
    """
    ...
    """
    Checks to see if the finder model is loading(training)
    (Allows non-blocking execution of the main display)
    """
    ...
    """
    Predicts which camera sees the mostactivity during the time period selected
    Displays the results in a dialog
    """
